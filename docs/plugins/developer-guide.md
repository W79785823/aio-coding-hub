...
5. 使用 `pnpm create-aio-plugin doctor` 和 `validate --strict` 做 package health、manifest 与入口文件校验。
6. 使用 `pnpm create-aio-plugin replay --explain` 在本地 fixture 上验证 hook 行为。
...
- 是否新增 capabilities 或贡献点。新增能力必须重新授权。
